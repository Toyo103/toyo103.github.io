...
- 생성자
    ...
    - Thread(ThreadGroup group, Runnable target) : 특정 스레드 그룹에 속하는 스레드를 생성하고, 실행할 Runnable 지정
...
### 5.4 Thread 클래스 주요 메소드
- 스레드의 속성을 확인하고 지정하는 메소드와 스레드의 상태를 통제하기 위한 메소드
    - void run()
...
- 그러나 만약 synchronized 메소드에 여러 로직이 있고, 그중 amount 인스턴수 변수를 한 줄에서만 다룬다고 하면 나머지 로직들을 수행하는데 불필요한 대기 시간이 발생하여 성능에 문제가 발생함. 이런 경우 synchronized 블록이 권장됨
    ...
    ```
- 위에 선언된 lock 객체는 synchronized 블록의 공동 열쇠와 같음. 코드를 보호하려면 여러 스레드가 반드시 같은 lock 객체를 공유해야 함
- 만약 인스턴스 변수가 2개 이상이고, 2개의 메소드에서 각각 다른 인스턴스 변수만 수정한다면 서로 독립적으로 실행되도록 해야 함. 같은 문지기 객체를 두 개의 메소드에서 사용하면 락이 같이 걸리므로 각 메소드별로 다른 문지기 객체를 만들어야 함
    ```
