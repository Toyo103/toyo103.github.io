...
- 7.1에서 정의한 Index, TermCounter 클래스를 레디스로 이동
    ...
    ```
- 레디스 데이터베이스에서 각 객체는 string 형의 유일한 키로만 식별하므로 두 종류의 객체(Index, TermCounter)가 있다면 이 둘을 구분할 수 있는 접두사를 추가해야 함
...
- 기존 Index 객체에서는 단어 별 TermCounter들의 집합을 인스턴스로 가졌으나 TermCounter는 특정 단어에 종속적인 객체가 아니므로 단어별 등장 url들의 집합으로 구성하는게 적합함
- n개의 페이지 인덱싱 후 고유한 검색어를 검색을 검색한다고 할 때 (getCounts 호출)
    - 맵을 생성 -> getURL 메소드 호출하여 해당 단어를 포함한 URL 집합을 가져옴 -> 집합에 있는 각 URL에 대해 getCount 메소드 호출하여 HashMap에 엔트리 추가
    - getURL 메소드의 smembers는 단어에 해당하는 url들을 모두 순회하여 클라이언트로 보내므로 최악의 경우 실행시간이 n에 비례하지만 대부분의 경우 특정 단어에 대한 URL 개수는 n보다 작음
- 한 페이지를 인덱싱하는데 소요되는 시간
    - 페이지의 DOM 트리를 순회하고 모든 TextNode 객체를 찾고 문자열을 검색어로 쪼갬 => 소요 시간은 페이지의 단어 개수에 비례
