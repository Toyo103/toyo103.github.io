...
- 레디스 데이터베이스에서 각 객체는 string 형의 유일한 키로만 식별되므로 두 종류 이상의 객체(Index, TermCounter)를 Redis에 넣는다면 이 둘을 구분할 수 있는 접두사를 추가해야 함
    - Index
        - Set에 키를 "URLSet : 단어", 값을 url 주소로 저장 => 해당 단어를 포함하는 url들의 Set
    - TermCounter
        - Hash에 첫번째 키를 "TermCounter : url 주소", 두번째 키를 단어, 값을 단어 등장 횟수로 저장 => 특정 url 주소 별 각 단어 등장 횟수 Map
...
- Collections.sort 메서드는 인자로 받는 리스트의 요소 자체가 Comparable를 구현하거나, Comparator 객체를 리스트와 함께 인자로 전달받는 두 가지 버전이 있음
